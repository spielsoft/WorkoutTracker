...
Use subagents for slices ... to ... . Read `AGENTS.md`, `ISSUES_MVP.md`, and `issues/MVP_prd.md` first. If additional project guidance files exist, read them before dispatching work.

Dispatch work in dependency order. Do not run dependent slices concurrently. Treat architecture review, validation gate, and cleanup slices in `ISSUES_MVP.md` as hard stops: use a stronger reviewer for the checkpoint, fix and commit any findings, then continue. If multiple subagents will touch the same checkout, serialize them; only run slices concurrently when they are independent and isolated in separate worktrees. Each completed slice must pass its relevant tests, update the `ISSUES_MVP.md` checklist, and commit its own changes before the next dependent slice starts.

Backend work must complete before GUI work begins. In this plan, do not start GUI slices until Slice 16 is complete. Google Sheets integration slices are provisionally HITL: first attempt AFK using the provided development sheet, but stop and request user action if local app authentication requires a browser/device login.
...
Read `AGENTS.md`, `ISSUES_MVP.md`, and `issues/MVP_prd.md` first. If additional project guidance files exist, read them before starting work.
