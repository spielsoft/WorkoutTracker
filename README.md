...
third-party behavior. A canned Google, Firebase, OAuth, Drive, or app-store
response must not be described as a behavior test. Use local tests to lock the
app-owned contract such as requested scopes, Drive query inputs, chooser
results, adapter inputs, and planned sheet writes. Use opt-in live
